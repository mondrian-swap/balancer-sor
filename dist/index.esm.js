...
import { WeiPerEther, Zero, AddressZero } from '@ethersproject/constants';
...
function _tokenInForExactBPTOut$1(amount, poolPairData) {
    ...
    **********************************************************************************************/
    ...
    const tokenBalancePercentageExcess = ONE$2.minus(currentWeight);
    // return amountIn
...
function _poolDerivativesBPT$1(
    ...
    const partial_D = ZERO$1.minus(minus_partial_D);
    let ans;
    if (is_first_derivative) {
        ...
        const partial_DD = ZERO$1.minus(D_P.times(n_times_nplusone).div(D));
        if (is_BPT_out) {
            ...
            if (wrt_out) {
                const D_prime = ZERO$1.minus(partial_x.div(partial_D));
...
function _spotPriceAfterSwapTokenInForExactBPTOut$2(amount, poolPairData) {
    ...
    ans = ONE$2.div(ans.times(feeFactor));
    return ans;
}
/////////
///  Derivatives of spotPriceAfterSwap
...
class StablePool {
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            if (amtScaled.eq(0)) return ZERO$1;
...
bn('1000000000000000000000000000000000000'); // 1e36
...
bn('130000000000000000000'); // 130e18
bn('-41000000000000000000'); // (-41)e18
...
ONE_18.minus(bn('100000000000000000')); // 1e18 - 1e17
ONE_18.plus(bn('100000000000000000')); // 1e18 + 1e17
bn(2).pow(254).idiv(ONE_20);
// 18 decimal constants
bn('128000000000000000000'); // 2ˆ7
bn('38877084059945950922200000000000000000000000000000000000'); // eˆ(x0) (no decimals)
bn('64000000000000000000'); // 2ˆ6
bn('6235149080811616882910000000'); // eˆ(x1) (no decimals)
// 20 decimal constants
bn('3200000000000000000000'); // 2ˆ5
bn('7896296018268069516100000000000000'); // eˆ(x2)
bn('1600000000000000000000'); // 2ˆ4
bn('888611052050787263676000000'); // eˆ(x3)
bn('800000000000000000000'); // 2ˆ3
bn('298095798704172827474000'); // eˆ(x4)
bn('400000000000000000000'); // 2ˆ2
bn('5459815003314423907810'); // eˆ(x5)
bn('200000000000000000000'); // 2ˆ1
bn('738905609893065022723'); // eˆ(x6)
bn('100000000000000000000'); // 2ˆ0
bn('271828182845904523536'); // eˆ(x7)
bn('50000000000000000000'); // 2ˆ(-1)
bn('164872127070012814685'); // eˆ(x8)
bn('25000000000000000000'); // 2ˆ(-2)
bn('128402541668774148407'); // eˆ(x9)
bn('12500000000000000000'); // 2ˆ(-3)
bn('113314845306682631683'); // eˆ(x10)
bn('6250000000000000000'); // 2ˆ(-4)
bn('106449445891785942956'); // eˆ(x11)

// Ported from Solidity:
bn(0);
const ONE$1 = bn('1000000000000000000'); // 10^18
bn(10000); // 10^(-14)
// Minimum base for the power function when the exponent is 'free' (larger than ONE)
bn('700000000000000000'); // 0.7e18
const add = (a, b) => {
...
const _calcBptOutPerMainIn = (
    ...
    const afterNominalMain = _toNominal(add(mainBalance, mainIn), params);
    const deltaNominalMain = sub$1(afterNominalMain, previousNominalMain);
...
const _calcBptInPerMainOut = (
    ...
    const previousNominalMain = _toNominal(mainBalance, params);
    const afterNominalMain = _toNominal(sub$1(mainBalance, mainOut), params);
    const deltaNominalMain = sub$1(previousNominalMain, afterNominalMain);
    const invariant = _calcInvariant(previousNominalMain, wrappedBalance);
...
const _calcWrappedOutPerMainIn = (mainIn, mainBalance, params) => {
    ...
    const previousNominalMain = _toNominal(mainBalance, params);
    const afterNominalMain = _toNominal(add(mainBalance, mainIn), params);
    return sub$1(afterNominalMain, previousNominalMain);
};
const _calcWrappedInPerMainOut = (mainOut, mainBalance, params) => {
    // Amount in, so we round up overall.
    const previousNominalMain = _toNominal(mainBalance, params);
    const afterNominalMain = _toNominal(sub$1(mainBalance, mainOut), params);
    return sub$1(previousNominalMain, afterNominalMain);
};
const _calcMainInPerBptOut = (
    ...
    const previousNominalMain = _toNominal(mainBalance, params);
    const invariant = _calcInvariant(previousNominalMain, wrappedBalance);
    const deltaNominalMain = divUp(mul(invariant, bptOut), bptSupply);
    const afterNominalMain = add(previousNominalMain, deltaNominalMain);
    const newMainBalance = _fromNominal(afterNominalMain, params);
    return sub$1(newMainBalance, mainBalance);
};
const _calcMainOutPerBptIn = (
    ...
    const previousNominalMain = _toNominal(mainBalance, params);
    const invariant = _calcInvariant(previousNominalMain, wrappedBalance);
    const deltaNominalMain = divDown(mul(invariant, bptIn), bptSupply);
    const afterNominalMain = sub$1(previousNominalMain, deltaNominalMain);
    const newMainBalance = _fromNominal(afterNominalMain, params);
    return sub$1(mainBalance, newMainBalance);
};
const _calcMainOutPerWrappedIn = (wrappedIn, mainBalance, params) => {
    // Amount out, so we round down overall.
    const previousNominalMain = _toNominal(mainBalance, params);
    const afterNominalMain = sub$1(previousNominalMain, wrappedIn);
    const newMainBalance = _fromNominal(afterNominalMain, params);
    return sub$1(mainBalance, newMainBalance);
};
const _calcMainInPerWrappedOut = (wrappedOut, mainBalance, params) => {
    // Amount in, so we round up overall.
    const previousNominalMain = _toNominal(mainBalance, params);
    const afterNominalMain = add(previousNominalMain, wrappedOut);
    const newMainBalance = _fromNominal(afterNominalMain, params);
    return sub$1(newMainBalance, mainBalance);
};
const _calcBptOutPerWrappedIn = (
    ...
    const newWrappedBalance = add(wrappedBalance, wrappedIn);
    const newInvariant = _calcInvariant(nominalMain, newWrappedBalance);
    const newBptBalance = divDown(
        mul(bptSupply, newInvariant),
        previousInvariant
    );
    return sub$1(newBptBalance, bptSupply);
};
const _calcBptInPerWrappedOut = (
    ...
    const nominalMain = _toNominal(mainBalance, params);
    const previousInvariant = _calcInvariant(nominalMain, wrappedBalance);
    const newWrappedBalance = sub$1(wrappedBalance, wrappedOut);
    const newInvariant = _calcInvariant(nominalMain, newWrappedBalance);
    const newBptBalance = divDown(
        mul(bptSupply, newInvariant),
        previousInvariant
    );
    return sub$1(bptSupply, newBptBalance);
};
const _calcWrappedInPerBptOut = (
    ...
    const nominalMain = _toNominal(mainBalance, params);
    const previousInvariant = _calcInvariant(nominalMain, wrappedBalance);
    const newBptBalance = add(bptSupply, bptOut);
    const newWrappedBalance = sub$1(
        divUp(mul(newBptBalance, previousInvariant), bptSupply),
        nominalMain
    );
    return sub$1(newWrappedBalance, wrappedBalance);
};
const _calcWrappedOutPerBptIn = (
    ...
    const nominalMain = _toNominal(mainBalance, params);
    const previousInvariant = _calcInvariant(nominalMain, wrappedBalance);
    const newBptBalance = sub$1(bptSupply, bptIn);
    const newWrappedBalance = sub$1(
        divUp(mul(newBptBalance, previousInvariant), bptSupply),
        nominalMain
    );
    return sub$1(wrappedBalance, newWrappedBalance);
};
...
const _toNominal = (real, params) => {
    ...
    if (real.lt(params.lowerTarget)) {
        const fees = mulDown(sub(params.lowerTarget, real), params.fee);
        return sub$1(real, fees);
    } else if (real.lte(params.upperTarget)) {
        return real;
    } else {
        const fees = mulDown(sub(real, params.upperTarget), params.fee);
        return sub$1(real, fees);
    }
};
const _fromNominal = (nominal, params) => {
    ...
    if (nominal.lt(params.lowerTarget)) {
        return divDown$1(
            add(nominal, mulDown(params.fee, params.lowerTarget)),
            add(ONE$1, params.fee)
        );
    } else if (nominal.lte(params.upperTarget)) {
        return nominal;
    } else {
        return divDown$1(
            sub$1(nominal, mulDown(params.fee, params.upperTarget)),
            sub$1(ONE$1, params.fee)
        );
    }
};
...
class LinearPool {
    ...
    constructor(
        ...
        this.poolType = PoolTypes.Linear;
        this.MAX_RATIO = parseFixed('10', 18); // Specific for Linear pool types
    ...
    _exactWrappedTokenInForMainOut(poolPairData, amount, exact) {
        try {
            // All values should use 1e18 fixed point
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
    ...
    _exactMainTokenInForWrappedOut(poolPairData, amount, exact) {
        try {
            // All values should use 1e18 fixed point
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
    ...
    _wrappedTokenInForExactMainOut(poolPairData, amount, exact) {
        try {
            // All values should use 1e18 fixed point
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
    ...
    _mainTokenInForExactWrappedOut(poolPairData, amount, exact) {
        try {
            // All values should use 1e18 fixed point
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
...
class PhantomStablePool {
        ...
        try {
            ...
            const returnWithFee = bnum(
                this.addSwapFeeAmount(
                    BigNumber$1.from(returnScaled.toString()),
                    poolPairData.swapFee
                ).toString()
            );
            // return human number
    ...
    subtractSwapFeeAmount(amount, swapFee) {
        ...
        const feeAmount = amount.mul(swapFee).div(WeiPerEther);
    ...
    addSwapFeeAmount(amount, swapFee) {
        ...
        const feeAmount = WeiPerEther.sub(swapFee);
        return amount.mul(WeiPerEther).add(feeAmount.sub(1)).div(feeAmount);
    }
}
...
const optimizeSwapAmounts = (
    ...
    for (let b = initialNumPaths; b <= paths.length; b++) {
        if (b != initialNumPaths) {
            ...
            swapAmounts.forEach((swapAmount, i) => {
                swapAmounts[i] = swapAmount.times(
                    ONE$2.minus(newSwapAmount.div(humanTotalSwapAmount))
                );
            });
        ...
        let totalReturnConsideringFees = ZERO$1;
...
const formatSwaps$1 = (
    ...
    let bestTotalReturn = ZERO$1; // Reset totalReturn as this time it will be
    ...
    if (bestTotalReturn.eq(0)) return [[], ZERO$1, ZERO$1];
...
function iterateSwapAmounts(
    ...
    let priceError = ONE$2; // Initialize priceError just so that while starts
    let prices = [];
...
function iterateSwapAmountsApproximation(
    ...
    let sumInverseDerivativeSPaSs = ZERO$1;
    let sumSPaSDividedByDerivativeSPaSs = ZERO$1;
    const SPaSs = [];
    ...
    swapAmounts.forEach((swapAmount, i) => {
        ...
        if (
            (iterationCount == 0 &&
                swapAmount.gte(ZERO$1) &&
                exceedingAmounts[i].lte(ZERO$1)) ||
            (iterationCount != 0 &&
                swapAmount.gt(ZERO$1) &&
                exceedingAmounts[i].lt(ZERO$1))
        ) {
    ...
    while (
        BigNumber.min.apply(null, swapAmounts).lt(ZERO$1) ||
        BigNumber.max.apply(null, exceedingAmounts).gt(ZERO$1)
    ) {
    ...
    let swapAmountsSumWithRoundingErrors = ZERO$1;
    swapAmounts.forEach((swapAmount, i) => {
    ...
    for (let i = 0; i < swapAmounts.length; ++i) {
        if (swapAmounts[i].gt(ZERO$1) && exceedingAmounts[i].lt(ZERO$1)) {
            if (
                swapAmounts[i].plus(roundingError).gt(ZERO$1) &&
                exceedingAmounts[i].plus(roundingError).lt(ZERO$1)
            ) {
...
function redistributeInputAmounts(
    ...
    let sumInverseDerivativeSPaSsForViableAmounts = ZERO$1;
    let sumInverseDerivativeSPaSsForNegativeAmounts = ZERO$1;
    let sumInverseDerivativeSPaSsForExceedingAmounts = ZERO$1;
    let sumNegativeOrExceedingSwapAmounts = ZERO$1;
    swapAmounts.forEach((swapAmount, i) => {
        // Amount is negative
        if (swapAmount.lte(ZERO$1)) {
            ...
            sumInverseDerivativeSPaSsForNegativeAmounts =
                sumInverseDerivativeSPaSsForNegativeAmounts.plus(
                    ONE$2.div(derivativeSPaSs[i])
                );
        }
        // Amount is above limit (exceeding > 0)
        else if (exceedingAmounts[i].gte(ZERO$1)) {
            sumNegativeOrExceedingSwapAmounts =
    ...
    swapAmounts.forEach((swapAmount, i) => {
        if (swapAmount.lte(ZERO$1)) {
            ...
            exceedingAmounts[i] = ZERO$1;
        } else {
            const deltaSwapAmount = sumNegativeOrExceedingSwapAmounts
                .times(ONE$2.div(derivativeSPaSs[i]))
    ...
    if (sumInverseDerivativeSPaSsForViableAmounts.isZero()) {
        if (sumNegativeOrExceedingSwapAmounts.lt(ZERO$1)) {
...
const getBestPaths = (
    ...
    if (totalSwapAmount.gt(sumLimitAmounts[sumLimitAmounts.length - 1])) {
        return [[], ZERO$1, ZERO$1, ZERO$1]; // Not enough liquidity, return empty
    }
    ...
    if (bestTotalReturn.eq(0)) return [[], ZERO$1, ZERO$1, ZERO$1];
...
const MULTIADDR = {
    ...
    2741: '0x1c6B64878def3d5fd1b156A3A642bBd6053C3f7b',
};
const VAULTADDR = {
    ...
    2741: '0x48cD08ad2065e0cD2dcD56434e393D55A59a4F64',
};
...
const Query = {
    ...
    106: queryWithLinear,
    2741: queryWithLinear,
};
// Returns all public pools
function fetchSubgraphPools(subgraphUrl, chainId = 1) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        const response = yield fetch(subgraphUrl, {
            method: 'POST',
            headers: {
...
function filterHopPools(tokenIn, tokenOut, hopTokens, poolsOfInterest) {
    ...
    for (let i = 0; i < hopTokens.length; i++) {
        let highestNormalizedLiquidityFirst = ZERO$1; // Aux variable to find pool with most liquidity for pair (tokenIn -> hopToken)
...
function getHighestLiquidityPool(
    ...
    let highestNormalizedLiquidity = ZERO$1;
